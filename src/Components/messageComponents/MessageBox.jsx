...
import ChatDetailsBox from '../chatDetails/ChatDetails';
...
const MessageBox = ({ chatId }) => {
    ...
    useEffect(() => {
		const getMessage = (chatId) => {
			...
			fetch(`https://chat-sync-backend-gin7.onrender.com/api/message/${chatId}`, {
				method: "GET",
				headers: {
    ...
    return (
        <>
			<div
				className="py-6 sm:px-6 px-3 w-full h-[7vh] font-semibold flex justify-between items-center bg-emerald-500 text-white cursor-pointer"
			...
			{isChatDetailsBox && (
				...
				>
					<div
						ref={chatDetailsBox}
						className="flex border border-slate-400 bg-red-800    overflow-hidden rounded-lg"
					>
						<ChatDetailsBox />
					</div>
				</div>
