...
// import { ChatShimmerSmall } from '../loading/Chattime';
...
// 		const token = localStorage.getItem("token");
// 		fetch('http://localhost:3000/api/chat/groupadd', {
// 			method: "POST",
// 			headers: {
// 				"Content-Type": "application/json",
// 				Authorization: `Bearer ${token}`,
...
const MemberAdd = () => {
  ...
  const [addUserName, setAddUserName] = useState('');
  const [addUserId, setAddUserId] = useState('');
  const [memberAddBox, setMemberAddBox] = useState(false); // Define state for member add box
  ...
  useEffect(() => {
    setSelectedUsers(
      users.filter((user) => {
        return (
          user.firstName.toLowerCase().includes(formik.values.search?.toLowerCase()) ||
          user.lastName.toLowerCase().includes(formik.values.search?.toLowerCase()) ||
          user.email.toLowerCase().includes(formik.values.search?.toLowerCase())
        );
      })
    );
  ...
  const handleAddUser = (userId, userName) => {
    ...
    setAddUserId(userId);
    setAddUserName(userName);
    setMemberAddBox(true); // Set memberAddBox to true when adding user
  };

  const handleAddUserCall = () => {
    dispatch(setLoading(true));
    const token = localStorage.getItem('token');
    fetch('https://chat-sync-backend-gin7.onrender.com/api/chat/groupadd', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
      ...
      .then((json) => {
        ...
        dispatch(addSelectedChat(json?.data));
        dispatch(setLoading(false));
        setMemberAddBox(false); // Reset memberAddBox state after adding user
      })
      .catch((err) => {
        console.log(err);
        toast.error(err.message);
  ...
  const handleBackClick = () => {
    setAddUserId('');
    setAddUserName('');
    setMemberAddBox(false); // Reset memberAddBox state on back click
  };
              ...
              <div className="flex gap-1">
                <div
                  onClick={() => {
                    setAddUserName('');
                    setAddUserId('');
                    setMemberAddBox(false); // Reset memberAddBox state on cancel
                  }}
                  className="border border-slate-600 p-1.5 w-fit font-normal outline-none rounded-md cursor-pointer bg-transparent active:bg-black/20"
                >
                  <VscError fontSize={19} />
